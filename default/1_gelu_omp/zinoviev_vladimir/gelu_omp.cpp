...
#if 0
int main() {
    const size_t input_size = 20000000;
    ...
    // Warming-up
    auto result1 = GeluOMPBase(input);
    auto result2 = GeluOMP(input);
    float diff = 0;
    for (int i = 0; i < result1.size(); ++i) {
        diff = std::max(std::abs((result1[i] - result2[i]) / result1[i]), diff);
    }
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        time_list1.push_back(duration1.count());
    }
    double time = *std::min_element(time_list.begin(), time_list.end());
    double time1 = *std::min_element(time_list1.begin(), time_list1.end());
